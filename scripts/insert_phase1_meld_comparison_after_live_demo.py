...
def insert_slide_before(prs, slide, before_index_0_based):
    ...
    # The new slide is always appended by python-pptx; move that appended id into place.
    sldId = sldIdLst[-1]
...
def add_comparison_slide(prs):
    ...
    add_text(slide, 0.42, 0.12, 11.9, 0.3, "MELD Result Comparison: MemoCMT vs Baseline vs Gated+Aux", size=18, bold=True, color="FFFFFF")
    add_text(slide, 12.0, 0.12, 0.8, 0.3, "", size=12, color="FFFFFF", align=PP_ALIGN.RIGHT)
...
def renumber_number_boxes(prs):
    for idx, slide in enumerate(prs.slides, start=1):
        ...
        for shp in slide.shapes:
            ...
            if not txt:
                continue
            # Slide-number box heuristics: digits only, upper-right corner.
            if re.fullmatch(r"\d+", txt) and float(shp.top) < 400000 and float(shp.left) > 9000000:
                shp.text = str(idx)


def main():
    prs = Presentation(str(PPTX_PATH))
    if any("MELD Result Comparison" in (shp.text_frame.text if getattr(shp, "has_text_frame", False) else "")
           for slide in prs.slides for shp in slide.shapes):
        print("Comparison slide already exists; only renumbering slide boxes.")
        renumber_number_boxes(prs)
        prs.save(str(PPTX_PATH))
        return

    slide = add_comparison_slide(prs)
    insert_slide_before(prs, slide, 24)  # after current Live Demo Results Summary
    renumber_number_boxes(prs)
    prs.save(str(PPTX_PATH))
    print(f"Inserted comparison slide into {PPTX_PATH}")
