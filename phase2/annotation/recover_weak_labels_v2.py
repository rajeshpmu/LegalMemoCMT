"""Complete missing evidence for conservative weak-label recovery.
...
def main() -> None:
    ...
    parser.add_argument("--vad-strong-valence", type=float, default=0.40)
    parser.add_argument("--vad-strong-arousal", type=float, default=0.45)
    parser.add_argument("--vad-moderate-valence", type=float, default=0.50)
    ...
    defaults = {
        ...
        "recovery_vad_non_neutral_support": "NONE",
        "recovery_courtroom_affect_points": "",
        "recovery_visual_points": "",
        "recovery_evidence_family_count": "",
        "recovery_penalty": "",
    ...
    for index, row in output.iterrows():
        ...
        valence_raw = text(row, "audio_valence", "odyssey_valence")
        arousal_raw = text(row, "audio_arousal", "audio_excitement", "odyssey_arousal")
        valence = number(row, "audio_valence", "odyssey_valence")
        arousal = number(row, "audio_arousal", "audio_excitement", "odyssey_arousal")
        if not valence_raw or not arousal_raw:
            vad_support, odyssey_points = "NONE", 0.0
        elif valence <= args.vad_strong_valence and arousal >= args.vad_strong_arousal:
            vad_support, odyssey_points = "STRONG", 1.0
        elif valence <= args.vad_moderate_valence or arousal >= args.vad_strong_arousal:
            vad_support, odyssey_points = "MODERATE", 0.5
        else:
            vad_support, odyssey_points = "NONE", 0.0
        affect = text(row, "proposed_courtroom_affect", "candidate_affect").upper()
        if affect in {"DISTRESSED", "AGITATED"}:
            affect_points = 1.0
        elif affect in {"TENSE", "HESITANT_UNCERTAIN", "GUARDED", "DEFENSIVE", "ASSERTIVE"}:
            affect_points = 0.5
        else:
            affect_points = 0.0
        video_reliable = yes(row, "video_emotion_reliable")
        visual_points = 1.0 if video_reliable and yes(row, "visual_non_neutral_support") else 0.0
        families = 1 + int(audio_polarity_points > 0) + int(odyssey_points > 0) + int(affect_points > 0) + int(visual_points > 0)
        ...
        output.at[index, "recovery_vad_non_neutral_support"] = vad_support
        ...
        output.at[index, "recovery_evidence_family_count"] = str(families)
    ...
    summary = {
        ...
        "notes": [
            ...
            "Odyssey is used as non-neutral activation support only; it does not decide an exact class.",
            "Courtroom affect gives strong support to DISTRESSED/AGITATED and contextual support to TENSE/HESITANT_UNCERTAIN/GUARDED/DEFENSIVE/ASSERTIVE.",
            "Visual evidence receives weight only when both video_emotion_reliable and visual_non_neutral_support are explicitly YES; speaker visibility alone is not emotion evidence.",
            "independent_evidence_family_count is reported separately because equal raw scores can have different evidential breadth.",
