...
bash "$ROOT_DIR/phase2/run_phase2_finetune.sh" "$PHASE2_SPLIT_MANIFEST" --device "$DEVICE" --init-checkpoint "$INIT_CKPT" --output-dir "$OUT_DIR"
bash "$ROOT_DIR/phase2/evaluate_phase2_checkpoint.sh" "$PHASE2_SPLIT_MANIFEST" "$OUT_DIR/best_model.pt" "$EVAL_JSON" test
